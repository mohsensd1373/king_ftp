...
function fileUploader(globalFiles, filePath, rowID, isFolder, isDrop) {
    ...
    if (file.size > upload_limit) {
        ...
        // Post form
        xmlhttp.open("POST", "media-upload.php?chromeless=1&tab=kig_ftp_my_file_tab&ftpAction=upload&filePath=" + filePath, true);
...
function wantchecked(id){
    //antuncheckedall();
    $('#'+id).prop('checked', true);
}
function antuncheckedall(){
...
function activateActionButtons(active, paste) {
    ...
    if (active == 1) {
        ...
        document.getElementById('actionButtonedit').disabled = false; // edit
        ...
        document.getElementById('actionButtonedit').disabled = true;
...
function showFileContextMenu(e, file, folder, isLin) {
    ...
    if (file != "")
        menuHTML += createContextMenuItem(lang_context_download, "checkFileChecked(); actionDownloadZip(); document.getElementById('id01').style.display='block';"  + "", 1);
...
function editfile(file, folder) {
     var vars = '&ftpAction=editfile';
...
document.getElementById('actionButton_empty_clipboard').disabled = false; 
...
function actionFunction_empty_clipboard(){
   globalClipboardAction = "";
   document.getElementById('actionButton_empty_clipboard').disabled = true; 
   document.getElementById('actionButtonPaste').disabled = true; 
}
...
function renamefilename(){
   var a=document.getElementById("renamename").value; 
   actionFunctionRename("", a);
}
...
function actionFunctionLogout() {
    ...
    document.location.href = 'media-upload.php?chromeless=1&tab=kig_ftp_my_file_tab&logout=1'
}
function actionFunctionhelp(){
    var vars = '&ftpAction=helpshow';
...
function submitToIframe(vars) {
    ...
    theForm.action = 'media-upload.php?chromeless=1&tab=kig_ftp_my_file_tab&' + vars;
    theForm.submit();

    // Reset values
    theForm.target = '';
    theForm.action = 'media-upload.php?chromeless=1&tab=kig_ftp_my_file_tab&';
    
    //$("#ajaxIframe").attr("src",'media-upload.php?chromeless=1&tab=kig_ftp_my_file_tab&' + vars);
...
function displayUploadButtons() {
    ...
    // Non-IE9 Upload File
    if (globalBrowser != "ie9-") {
        html += '<button type="button" id="upload_files"  onClick="fileChoose(event)" class=" w3-button w3-teal ' + adjustButtonWidth(lang_btn_upload_files) + '"> <i class="fa  fa-download"></i> <i class="fa  fa-file"></i></button>';
    }
    ...
    html += '<div class="floatLeft10">';
    html += '<button class=" w3-button w3-teal inputButton" type="button" style="width: 50px;" onclick="processFileUploads(event,0,0,\'\')" class="' + adjustButtonWidth(lang_btn_upload_repeat) + '"><i class="fa fa-retweet"></i> </button>';
    html += '</div>';
    html += '</div>';
    ...
    if (globalBrowser == "chrome") {
        ...
        html += '<button type="button" id="upload_folder"  onClick="dirChoose(event)" class=" w3-button w3-teal ' + adjustButtonWidth(lang_btn_upload_folder) + '"><i class="fa  fa-download"></i> <i class="fa  fa-folder"></i></button>';
